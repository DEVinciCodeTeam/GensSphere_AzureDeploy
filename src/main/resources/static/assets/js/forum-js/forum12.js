getJsonFromApi("forum122Posts.json");
...
function appendObjectToLocalStorage(allData) {
  ...
  sessionStorage.setItem("forum12Posts", JSON.stringify(element));
  updateForumObject("forum12Posts");
}
...
function onConnected() {
    ...
    stompClient.subscribe('/topic/public12', onMessageReceived);

    stompClient.subscribe('/topic/public/reply12', onMessageReceived_Reply);
  ...
  if (postInput === "") {
    ...
    return;
      ...
      if(postInput && stompClient) {
          ...
          stompClient.send('/app/chat.send12', {}, JSON.stringify(chatMessage));
...
function addReply(event){
  ...
  if (replyText === "") {
    ...
    if(replyText && stompClient) {
        ...
        stompClient.send('/app/chat.reply12', {}, JSON.stringify(chatMessage));
        replyText.value = '';
    }
    event.preventDefault();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getDataFromLocalStorage() {
    const storedData = sessionStorage.getItem("forum12Posts");
