getJsonFromApi("forum111Posts.json");
...
function appendObjectToLocalStorage(allData) {
  ...
  sessionStorage.setItem("forum11Posts", JSON.stringify(element));
  updateForumObject("forum11Posts");
}
...
function onConnected() {
    ...
    stompClient.subscribe('/topic/public11', onMessageReceived);

    stompClient.subscribe('/topic/public/reply11', onMessageReceived_Reply);
  ...
  if (postInput === "") {
    ...
    return;
      ...
      if(postInput && stompClient) {
          ...
          stompClient.send('/app/chat.send11', {}, JSON.stringify(chatMessage));
...
function addReply(event){
  ...
  if (replyText === "") {
    ...
    if(replyText && stompClient) {
        ...
        stompClient.send('/app/chat.reply11', {}, JSON.stringify(chatMessage));
        replyText.value = '';
    }
    event.preventDefault();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getDataFromLocalStorage() {
    const storedData = sessionStorage.getItem("forum11Posts");
