getJsonFromApi("forum5Posts.json");
...
function appendObjectToLocalStorage(allData) {
  ...
  sessionStorage.setItem("forum5Posts", JSON.stringify(element));
  updateForumObject("forum5Posts");
}
...
function onConnected() {
    ...
    stompClient.subscribe('/topic/public/reply5', onMessageReceived_Reply);
  ...
  if (postInput === "") {
    ...
    return;
      ...
      if(postInput && stompClient) {
          ...
          stompClient.send('/app/chat.send5', {}, JSON.stringify(chatMessage));
...
function addReply(event){
  ...
  if (replyText === "") {
    ...
    if(replyText && stompClient) {
        ...
        stompClient.send('/app/chat.reply5', {}, JSON.stringify(chatMessage));
        replyText.value = '';
    }
    event.preventDefault();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getDataFromLocalStorage() {
    const storedData = sessionStorage.getItem("forum5Posts");
