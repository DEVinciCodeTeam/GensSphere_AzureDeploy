...
function appendObjectToLocalStorage(allData) {
  ...
  sessionStorage.setItem("forum10Posts", JSON.stringify(element));
  updateForumObject("forum10Posts");
}
...
function onConnected() {
    ...
    stompClient.subscribe('/topic/public10', onMessageReceived);

    stompClient.subscribe('/topic/public/reply10', onMessageReceived_Reply);
  ...
  if (postInput === "") {
    ...
    return;
      ...
      if(postInput && stompClient) {
          ...
          stompClient.send('/app/chat.send10', {}, JSON.stringify(chatMessage));
...
function addReply(event){
  ...
  if (replyText === "") {
    ...
    if(replyText && stompClient) {
        ...
        stompClient.send('/app/chat.reply10', {}, JSON.stringify(chatMessage));
        replyText.value = '';
    }
    event.preventDefault();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getDataFromLocalStorage() {
    const storedData = sessionStorage.getItem("forum10Posts");
