getJsonFromApi("forum8Posts.json");
...
function appendObjectToLocalStorage(allData) {
  ...
  sessionStorage.setItem("forum8Posts", JSON.stringify(element));
  updateForumObject("forum8Posts");
}
...
function onConnected() {
    ...
    stompClient.subscribe('/topic/public/reply8', onMessageReceived_Reply);
  ...
  if (postInput === "") {
    ...
    return;
      ...
      if(postInput && stompClient) {
          ...
          stompClient.send('/app/chat.send8', {}, JSON.stringify(chatMessage));
...
function addReply(event){
  ...
  if (replyText === "") {
    ...
    if(replyText && stompClient) {
        ...
        stompClient.send('/app/chat.reply8', {}, JSON.stringify(chatMessage));
        replyText.value = '';
    }
    event.preventDefault();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getDataFromLocalStorage() {
    const storedData = sessionStorage.getItem("forum8Posts");
