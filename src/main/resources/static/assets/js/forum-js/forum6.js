getJsonFromApi("forum6Posts.json");
...
function appendObjectToLocalStorage(allData) {
  ...
  sessionStorage.setItem("forum6Posts", JSON.stringify(element));
  updateForumObject("forum6Posts");
}
...
function onConnected() {
    ...
    stompClient.subscribe('/topic/public/reply6', onMessageReceived_Reply);
  ...
  if (postInput === "") {
    ...
    return;
      ...
      if(postInput && stompClient) {
          ...
          stompClient.send('/app/chat.send6', {}, JSON.stringify(chatMessage));
...
function addReply(event){
  ...
  if (replyText === "") {
    ...
    if(replyText && stompClient) {
        ...
        stompClient.send('/app/chat.reply6', {}, JSON.stringify(chatMessage));
        replyText.value = '';
    }
    event.preventDefault();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getDataFromLocalStorage() {
    const storedData = sessionStorage.getItem("forum6Posts");
